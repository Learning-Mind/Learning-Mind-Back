from sqlalchemy.ext.asyncio import AsyncSession as SqlAsyncSession
from fastapi import APIRouter, Depends
from app.db.session import get_psql_session
from app.schema.api import APIResponseSchema
from app.service.roadmap import RoadmapService

from app.schema.roadmap import CreateRoadmapNodeSchema, CreateRoadmapSchema, RoadmapNodeResponseSchema, RoadmapResponseSchema


router = APIRouter()

# 특정 로드맵 조회 API
@router.get(
    path="/{roadmap_id}",
    summary="id로 로드맵 조회",
    description="roadmap_id로 로드맵을 조회합니다.",
    response_model=APIResponseSchema[RoadmapResponseSchema],
)
async def get_roadmap_item(
    roadmap_id: int,
    se: SqlAsyncSession = Depends(get_psql_session)
):
    result = await RoadmapService(se).get_roadmap_item_by_id(roadmap_id=roadmap_id)

    return APIResponseSchema(
        success=True,
        payload=RoadmapResponseSchema(
            roadmap_id=result.roadmap_id,
            title=result.title,
            nodes=result.nodes
        )
    )

# 특정 로드맵을 트리 데이터로 조회 API
@router.get(
    path="/{roadmap_id}/tree",
    # summary="id로 로드맵 조회",
    # description="roadmap_id로 로드맵을 조회합니다.",
    # response_model=RoadmapResponseSchema,
)
async def get_roadmap_item(
    roadmap_id: int,
    se: SqlAsyncSession = Depends(get_psql_session)
):
    result = await RoadmapService(se).get_roadmap_item_by_tree(roadmap_id=roadmap_id)

    # return RoadmapResponseSchema(
    #     code=0,
    #     message="",
    #     roadmap_id=result.roadmap_id,
    #     title=result.title,
    #     nodes=result.nodes
    # )


# 로드맵 생성 API
@router.post(
    path="/write",
    summary="로드맵 생성",
    description="로드맵을 생성합니다.",
    response_model=APIResponseSchema[RoadmapResponseSchema],
)
async def write_roadmap(
    body: CreateRoadmapSchema,
    se: SqlAsyncSession = Depends(get_psql_session)
) -> APIResponseSchema:
    result = await RoadmapService(se).create_roadmap(body)
    return APIResponseSchema(
        success=True,
        payload=RoadmapResponseSchema(
            roadmap_id=result.roadmap_id,
            title=result.title
        )
    )

# 로드맵 노드 생성 API
@router.post(
    path="/node/write",
    response_model=APIResponseSchema[RoadmapNodeResponseSchema]
)
async def write_roadmap_node(
    body: CreateRoadmapNodeSchema,
    se: SqlAsyncSession = Depends(get_psql_session)
):
    result = await RoadmapService(se).create_roadmap_node(roadmap_node=body)
    return APIResponseSchema(
        success=True,
        payload=RoadmapNodeResponseSchema(
            node_id=result.node_id,
            roadmap_id=result.roadmap_id,
            title=result.title,
            parent_id=result.parent_id,
            order_in_parent=result.order_in_parent,
        )
    )

# TODO 로드맵 노드 이름 수정 API

# TODO 로드맵 parent 수정 API